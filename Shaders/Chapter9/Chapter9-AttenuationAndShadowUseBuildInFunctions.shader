//Blinn-Phong 光照模型 (逐片元) + 前向渲染（处理不同类型的光源，不再只是平行光）+ 统—管理光照衰减和阴影（使用宏）
// 
//为前向渲染定义了 Base Pass 和 AdditionalPass 来处理多个光源
// 
//使用UNITY_LIGHT_ATTENUATION同时计算把光照衰减因子和阴影值
    ...
    SubShader
    {
        ...
        {   // Pass for ambient light & first pixel light (directional light)
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                //它会将光照衰减和阴影值相乘后的结果存储到第一参数中。注意到，我们并没有在代码中声明第一个参数atten, 
                //这是因为UNITY_LIGHT_ATTENUATION 会帮我们声明这个变量

                // 【把阴影值shadow 和 漫反射、高光反射颜色相乘】
                //return fixed4(ambient + (diffuse + specular) * atten * shadow, 1.0);
                return fixed4(ambient + (diffuse + specular) * atten, 1.0); //atten已经是光照衰减×阴影值了
            }
        ...
        {   // Pass for other pixel lights
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                //不包含环境光项
                //【把阴影值shadow 和 漫反射、高光反射颜色相乘】
                return fixed4((diffuse + specular) * atten, 1.0); //atten已经是光照衰减×阴影值了
            }
