//漫反射光照模型 (逐片元)（不含高光反射） + 主纹理 + 透明度混合（双面渲染）
...
Shader "Unity Shaders Book/Chapter 8/Alpha Blend"
{
    ...
    SubShader
    {
        ...
        //透明度混合 的 双面渲染，需要把工作分成2个Pass，第一个只渲染背面，第二个只渲染正面，
        //由于Unity会顺序执行SubShader中的各个Pass，因此我们可以保证背面总是在正面被渲染之前渲染，
        //从而可以保证正确的深度渲染关系（对于半透明物体：从远到近）
        Pass
        {
